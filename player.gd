	...
	if is_dead == false and is_sinking == false:
		...
		if get_slide_count() > 0:
			for i in range(get_slide_count()):
				if "enemy" in get_slide_collision(i).collider.name || "Water" in get_slide_collision(i).collider.name|| "static_rotating_trap" in get_slide_collision(i).collider.name:
					dead()
